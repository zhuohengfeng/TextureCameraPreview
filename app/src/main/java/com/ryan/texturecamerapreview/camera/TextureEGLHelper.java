...
import com.ryan.texturecamerapreview.renderer.CameraTextureRenderer;
...
public class TextureEGLHelper extends HandlerThread implements SurfaceTexture.OnFrameAvailableListener {
    ...
    // 1. 创建一个OES TextureID
    // 2. 生成一个SurfaceTexture， new SurfaceTexture(mOESTextureId);
    // 3. 把这个SurfaceTexture设置给camera
    // 4. camera设置数据回调setOnFrameAvailableListener
    // 5. 在onFrameAvailable回调中，去绘制TextureView。
    // 所以TextureView其实不是和camera直接交换的，而是通过OESTextureID得到纹理后，在通过opengl绘制到TextureView上(这里需要先建立EGL，会传入TextureView的surface来创建)
